...
def Main(processNo,devices):
    ...
    print(Time.nowtime(),"进入",processNo,"进程,devicename=",devices)
    isconnect=""
    try:
    ...
    if isconnect=="Pass":
        try:
            print(Time.nowtime(), "设备", devices, "开始安装apk")
...
def main():
    ...
    print(Time.nowtime(), "测试开始")
    try:
        pool = multiprocessing.Pool(processes = len(devicesList))
        print(Time.nowtime(), "启动进程池")
        results = []
        for i in range(len(devicesList)):
            pool.apply_async(Main, (i,devicesList[i],))#根据设备列表去循环创建进程，对每个进程调用下面的Main方法。
        pool.close()
        pool.join()
        print(Time.nowtime(), "进程回收完毕")
        print(Time.nowtime(), "测试结束")
    except AirtestError as ae:
        print(Time.nowtime(), "Airtest发生错误" + ae)
    except PocoException as pe:
        print(Time.nowtime(), "Poco发生错误" + pe)
    except Exception as e:
        print(Time.nowtime(), "发生未知错误" + e)
